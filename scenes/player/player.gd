...
## Supports Mobile Touch Joystick + Right-screen Camera Drag + PC Controls
## Features weapon swap/pickup, melee punching, slow-motion bullet shooting, and time dilation.
...
@onready var pickup_detector: Area3D = $Head/PickupDetector
...
func do_primary_action() -> void:
	...
	# 1. Check if we can pick up / swap a nearby weapon on the floor
	if try_interact_pickup():
		return
	
	# 2. If holding a weapon, shoot it
...
func do_punch() -> void:
	...
	if not hit_enemy and punch_area:
...
func try_interact_pickup() -> bool:
	var target_weapon: Node3D = null
	
	# 1. Check aim raycast first
	if aim_ray.is_colliding():
		var col = aim_ray.get_collider()
		if col and col.is_in_group("weapon") and col != held_weapon and col.has_method("equip_to_hand"):
			target_weapon = col
	
	# 2. If ray didn't hit, check nearby pickup detector sphere
	if target_weapon == null and pickup_detector:
		var bodies = pickup_detector.get_overlapping_bodies()
		for b in bodies:
			if b.is_in_group("weapon") and b != held_weapon and b.has_method("equip_to_hand"):
				target_weapon = b
				break
	
	if target_weapon != null:
		# If already holding a weapon, swap: drop the old one!
		if held_weapon and is_instance_valid(held_weapon):
			var throw_dir = -camera.global_transform.basis.z + Vector3.UP * 0.3
			held_weapon.drop_or_throw(hand.global_transform, throw_dir * 4.0)
			held_weapon = null
...
func try_pickup_weapon(weapon_node: Node3D) -> void:
	if weapon_node != held_weapon and weapon_node.has_method("equip_to_hand"):
		if held_weapon == null:
